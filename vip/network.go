...
func NewNetlinkNetworkConfigurator(networkAddress string, networkInterface string) (result NetlinkNetworkConfigurator, error error) {
	cidr := networkAddress + "/32"

	ip, _, error := net.ParseCIDR(cidr)
	if error != nil {
		error = errors.Wrapf(error, "could not parse ip '%s'", networkAddress)

		return
	}

	result = NetlinkNetworkConfigurator{_interface: networkInterface, ip: ip}

	result.address, error = netlink.ParseAddr(cidr)
	if error != nil {
		error = errors.Wrapf(error, "could not parse address '%s'", networkAddress)

		return
	}

	result.link, error = netlink.LinkByName(networkInterface)
	if error != nil {
		error = errors.Wrapf(error, "could not get link for interface '%s'", networkInterface)
